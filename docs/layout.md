...
**Размещение пинов ввода-вывода**

7. Запустите скрипт размещения пинов, выполнив в терминал OpenROAD команду:

```tcl
source ../../scr/layout/pins_placement.tcl
```

В случае, если вы ознакамливались с памяткой по TCL, рекомендуем предварительно просмотреть содержание соответствующего скрипта.
Скрипт равномерно распределит входные пины по левой стороне блока, а выходные по правой стороне. Таким образом будет обеспечиваться более равномерное размещение стандартных ячеек при дальнейшем размещении.
На рисунке 16 приведен результат выполнения скрипта.

![][pins]

[pins]: images/pins.png

*Если после выполнения скрипта пины не отобразились, немного приблизьте блок. Для этого зажмите клавишу Ctrl на клавиатуре и приближайте/удаляйте колесиком мышки.  Чтобы быстро вернуться к просмотру всего блока можно нажать клавишу f.
